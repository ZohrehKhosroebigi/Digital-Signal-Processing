...
function Speech_MFCC
load TrainSet
load TestSet
if ~(exist('.\Data','file'))
    mkdir('.\Data');
end
if  ~(exist('.\Data\DataTrain.mat'))
    DataTrain=[];
    DataTrainCMS=[];
    ...
    for i = 1:10
        for j = 1:190
            maxlentrain = max(maxlentrain , numel( TrainSet{i,j}));
        end
        
    end
    for i = 1:10
        for j = 1:190
            TrainSet{i,j} = padarray(TrainSet{i,j} , [(maxlentrain - numel(TrainSet{i,j})) 0] , eps , 'post');
        end
        for j = 1:30
            TestSet{i,j} = padarray(TestSet{i,j} , [maxlentrain - numel(TestSet{i,j}) 0] , eps , 'post');
        end
    end
    for i=1:10
        for j=1:size(TrainSet,2)
            c=c+1;
            VecCep=MFCC(TrainSet{i,j});
            
            CMSCep=VecCep;
            for k=1:size(CMSCep,1)
             CMSCep(k,:)= CMSCep(k,:)-repmat( mean(CMSCep(k,:) ),1,size(CMSCep,2));
            end
            CMSCep=CMSCep(:)';
            CMSCep(:,end+1)=1;%biose
            CMSCep(:,end+1)=i;%class
            
            VecCep=VecCep(:)';                   
            VecCep(:,end+1)=1;%biose
            VecCep(:,end+1)=i;%class
            DataTrain(c,:)=VecCep;
            DataTrainCMS(c,:)=CMSCep;
        end
    end
    save('.\Data\DataTrain.mat','DataTrain');
      save('.\Data\DataTrainCMS.mat','DataTrainCMS');
  
end
if  ~(exist('.\Data\DataTest.mat'))
    DataTest=[];
    DataTestCMS=[];
    c=0;
    for i=1:10
        for j=1:size(TestSet,2)
            ...
            VecCep(:,end+1)=i;
            DataTest(c,:)=VecCep;
            DataTestCMS(c,:)=CMSCep;
        end
    end
       save('.\Data\DataTestCMS.mat','DataTestCMS');

    save('.\Data\DataTest.mat','DataTest');
end
end
function [VecCep] = MFCC(WaveFile)
...
if ischar(WaveFile)
    ...
    Bits=16;
    [S_in,Fs]=audioread(WaveFile);   % Read wave file
    
    
    %         [S_in,Fs,Bits]=wavread(WaveFile);   % Read wave file
else
    S_in = WaveFile;
    Fs = 16000;
    Bits = 16;
...
if (1)                      % Compute Delta and Delta-Delta
