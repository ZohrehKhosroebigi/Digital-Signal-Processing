...
clear All
FeatureExtract();
load '.\Data\DatatrainP10';
load '.\Data\DataTestP10';
...
while(Stop~=1)
    ...
    for i=1:length(shuffle)
        ...
        for j=1:NumberNeronHidden
            z_in(j)=sum(V(j,:).*(Datatrain(shuffle(i),1:11)));
        ...
        for k=1:NumberNeronHidden
            DeltaV(k,:)=Alpha*delta_1(:,k)*Datatrain(shuffle(i),1:11);
        end
        %--------------------------------update weights
        W=DeltaW+W;
        V=DeltaV+V;
...
if  ~(exist('.\Data\DataTrainP10.mat'))
    for i = 1:10
        ...
        for j=3:size(folder,1)
            ...
            lensegment=length(Signal)/10;
            s=s+1;
            for k=1:10
                Segment = Signal(round((k-1)*lensegment+1):round(k*lensegment));
                Datatrain(s,k) = FuncPower(Segment);
    ...
    save('.\Data\DataTrainP10.mat','Datatrain');
end
s=0;
if~(exist('.\Data\DataTestP10.mat'))
    for i = 1:10
        ...
        for j=3:size(folder,1)
            ...
            for k=1:10
                Segment = Signal(round((k-1)*lensegment+1):round(k*lensegment));
                Datatest(s,k) = FuncPower(Segment);
            end
    ...
    Datatest(:,11)=1;
    save('.\Data\DataTestP10.mat','Datatest');
