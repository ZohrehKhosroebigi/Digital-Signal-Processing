...
clear All
FeatureExtract();
load '.\Data\DatatrainP50';
load '.\Data\DataTestP50';
...
while(Stop~=1)
    ...
    for i=1:length(shuffle)
        ...
        for j=1:NumberNeronHidden
            z_in(j)=sum(V(j,:).*(Datatrain(shuffle(i),1:51)));
        ...
        for k=1:NumberNeronHidden
            DeltaV(k,:)=Alpha*delta_1(:,k)*Datatrain(shuffle(i),1:51);
        end
        %--------------------------------update weights
        W=DeltaW+W;
        V=DeltaV+V;
...
if  ~(exist('.\Data\DataTrainP50.mat'))
    for i = 1:10
    ...
    save('.\Data\DataTrainP50.mat','Datatrain');
end
s=0;
if~(exist('.\Data\DataTestP50.mat'))
    for i = 1:10
    ...
    Datatest(:,51)=1;
    save('.\Data\DataTestP50.mat','Datatest');
